...
def main():
    ...
    even_better_draw_circles(rg.Point(100,50),10,15,'pink',9)
...
def better_draw_circles(point, kylie):
    """
    ...
    for k in range(1, 11):  # k becomes 1, 2, 3, ... 10
        ...
        # Go DOWN 15 pixels, ending up pointing east again
        turtle.right(90)
        turtle.forward(kylie)
        turtle.left(90)

        turtle.pen_down()
        turtle.draw_circle(kylie * k)  # Radius 15, 30, 45, 60, ...
...
def even_better_draw_circles(point, kendall, number, color, thickness):
    turtle=rg.SimpleTurtle()
    ...
    for k in range(number):
        ...
        turtle.right(90)
        turtle.forward(kendall)
        turtle.left(90)
        turtle.pen_down()
        turtle.draw_circle(kendall * k)
